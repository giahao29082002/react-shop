...
const UserProfile = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const responsive =await editProfile(updatedUser).unwrap();
            console.log(responsive)
            dispatch(setUser(responsive.user))
            localStorage.setItem('user',JSON.stringify(responsive.user))
           alert('Profile updated Successfully!')
        } catch (error) {
            console.error("Failed to updated",error);
            alert("Failed to updated. Pls try Agian")
        }
                ...
                isModalOpen && (
                    <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-90 z-50'>
                        <div className='bg-white p-6 rounded-lg md:w-96 max-w-xl mx-auto relative'>
                            <button
                                onClick={() => setIsModalOpen(false)} // Đóng modal khi nhấn nút
                            ...
                            <form onSubmit={handleSubmit} >
                                ...
                                <div className='mb-4'>
                                    ...
                                    <input type='text' name='profileImage' value={formData?.profileImage}
                                        onChange={handleChange} placeholder='profileImage url'
                                        className='mt-1 p-2 w-full border border-gray-300 rounded-md shadow-sm' required
                                    />
                                </div>
                                <div className='mb-4'>
                                    <label htmlFor='bio' className='block text-sm font-medium text-gray-700'>Write your Bio</label>
                                    <textarea name='bio' rows='3' className='mt-1 p-2 w-full border border-gray-300 rounded-md shadow-sm'
                                ...
                                <div className='mb-4'>
                                    ...
                                    <input type='text' name='profession' value={formData?.profession}
                                        onChange={handleChange} placeholder='profession'
                                        className='mt-1 p-2 w-full border border-gray-300 rounded-md shadow-sm' required
                                    />
                                </div>
                                <button className={`mt-4 w-full bg-blue-500 text-white py-2 px rounded-md ${isLoading ?
                                    'opacity-50 cursor-not-allowed' : ''}`
                                } type='submit' disabled={isLoading}>
                                    {isLoading ? 'Saving...' : 'Save Changes'}</button>
                                {isError && <p className='mt-2 text-red-500'>Failed to update ... Try agian pls</p>}
                                {isSuccess && <p className='mt-2 text-green-500'>Profile success</p>}
                            </form >
                        </div>
                    </div>
                )
