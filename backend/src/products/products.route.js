...
router.get("/", async (req, res) => {
    try {
        const { category, size, minPrice, maxPrice, page = 1, limit = 10 } = req.query;
        ...
        console.error("Error feelching product", error);
        res.status(500).send({ message: "Error feelching product" })
    }
})
...
router.get("/:id", async (req, res) => {
    try {
        ...
        console.error("Error feelching product", error);
        res.status(500).send({ message: "Fail to fetch new product" })
    }
})
